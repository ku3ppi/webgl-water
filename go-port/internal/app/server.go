...
import (
	...
	"github.com/webgl-water-go/go-port/internal/assets"
	"github.com/webgl-water-go/go-port/internal/state"
)
...
func (s *Server) handleAssetFile(w http.ResponseWriter, r *http.Request) {
	...
	// Try serving from the webgl-water root directory (where the original PNG files are)
	// Working directory is go-port, so go up one level to webgl-water root
